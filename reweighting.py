...
def reweight_chadsvasc_performance(df, weights):
    ...
    for score, group in grouped:
        ...
        if weighted_years > 0:
            ...
            # Simple bootstrap for confidence intervals (a better approach would use proper weighted statistics)
            n_bootstrap = 1000
            bootstrap_rates = []
...
def plot_weighted_vs_original(results_df, unweighted_df=None):
    ...
    plt.errorbar(results_df['CHADS-Vasc'], results_df['Weighted Stroke Rate'], 
                 yerr=[results_df['Weighted Stroke Rate'] - results_df['95% CI Lower'], 
                       results_df['95% CI Upper'] - results_df['Weighted Stroke Rate']], 
