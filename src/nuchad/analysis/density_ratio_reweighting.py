...
def embed_png_metadata(png_path, metadata):
    """
    Embed metadata into a PNG file using PIL/Pillow.
    ...
    try:
        ...
        with Image.open(png_path) as img:
            ...
            # Add individual metadata fields as text chunks
            for key, value in metadata.items():
                png_info.add_text(f"nuchad.{key}", str(value))
            
            # Add full metadata as JSON
            png_info.add_text("nuchad.metadata", metadata_json)
            
            # Add generation info
            png_info.add_text("Software", "nuchad-analysis")
            png_info.add_text("Comment", "Generated by nuchad density ratio reweighting analysis")
            
            # Save the image with metadata
            img.save(png_path, pnginfo=png_info)
        
        print(f"Metadata embedded in {png_path}")
        
    except ImportError:
        print("Warning: PIL/Pillow not available. Metadata not embedded.")
    except Exception as e:
        print(f"Warning: Could not embed metadata in {png_path}: {e}")
...
def plot_results(results, save_path, metadata=None):
    """
    ...
    Args:
        ...
        metadata: Optional metadata dictionary to embed in PNG
    """
    # Extract data
...
def save_results_to_markdown(results, df_weighted, filepath, metadata=None):
    """
    ...
    with open(filepath, 'w') as f:
        ...
        if metadata:
            ...
            f.write("## Analysis Metadata\n\n")
            f.write("No metadata provided (pattern should be to include stuff \
                     like data file name, data file creation date, analysis run date, \
                     number of patients, analysis type, effective sample size, and results \
                    directory)\n\n")
...
def plot_weight_distribution(df_weighted, save_path, metadata=None):
    """
...
def perform_reweighting_analysis(df=None):
    ...
    try:
        ...
        # Try to determine the data file name
        data_file_name = "unknown"
        data_file_creation_date = "unknown"
        
        # Check if we can determine the data file path
        data_path = get_data_path() / 'random_nuchad.csv'  # Default
...
def main():
    ...
    save_results_to_markdown(results, df_weighted, str(results_path), metadata)
