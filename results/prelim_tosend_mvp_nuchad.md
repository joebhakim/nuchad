...
**Outcome**: Binary stroke event within 1 year of follow-up (`stroke_1Y == 1`).

**Density ratio reweighting**: Propensity score-based method to adjust UK population to match original CHADS-VAsC development population. Weights computed as w(x) = P(S=source|x) / P(S=target|x) where S indicates population source. Each UK patient weighted by likelihood ratio of belonging to source vs target population based on age, gender, comorbidities.

**AUC calculation**: Area under ROC curve for binary stroke prediction. CHADS-VAsC scores mapped to published 1-year risk rates (Lip et al.); logistic regression fitted directly on UK cohort features.
...
**Bootstrap 95% CI**: [0.098, 0.110] for logistic model vs CHADS-VAsC
